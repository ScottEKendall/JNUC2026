...
# NOTE: you cannot use the $HOME variable in a support.app extension script because the extension runs as root and $HOME will resolve to /var/root which is not what we want. 
...
# Set the loading key to true to trigger the loading animation in the Support App while we retrieve the password age and calculate days left until expiration
...
# This file is created from the script that retrieves the password age and last changed date from our Entra Server and writes it to the local users com.GiantEagleEntra.plist. 
# If this file doesn't exist or the values can't be read for some reason, we will default to showing (passwordLimit) days left until expiration so that we at least get some output
